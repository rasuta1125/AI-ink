import { callOpenAIWithRetry, CaptionSchema } from '../_lib/openai'
import { validateCaptionRequest } from '../_lib/validation'
import { getFallbackCaption, hasForbiddenWords } from '../_lib/fallbacks'
...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    const validatedInput = validateCaptionRequest(body)
...
{"captions":[
  ...
  {"text":"...","parts":{...}}
    ...
    try {
      // OpenAI API 呼び出し
      const result = await callOpenAIWithRetry(prompt, apiKey, CaptionSchema)
      
      // 禁止語チェック
      const filteredCaptions = result.captions.filter((caption: any) => 
      ...
      return new Response(JSON.stringify({
        ...
        meta: {
          ...
          filtered_count: result.captions.length - filteredCaptions.length
        }
      }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
